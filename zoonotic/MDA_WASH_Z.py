...
# State order: [S_C, E_C, I_C, S_A, E_A, I_A, S_D, E_DA, I_DA, L_A]
...
def main():
    ...
    for idx, row in eq.iterrows():
        ...
        y0 = np.array([
            ...
            row['S_D_final'], row['E_DA_final'], row['I_DA_final'],
            row['L_A_final']
        ], dtype=float)
            ...
            if t_eval_seg.size == 0:
                ...
                for j in range(sol.t.size):
                    ...
                    state_rows.append({
                        ...
                        'S_D': state[IDX['S_D']], 'E_DA': state[IDX['E_DA']], 'I_DA': state[IDX['I_DA']],
                        'L_A': state[IDX['L_A']], 
                    })
