...
"""
Model for zoonotic transmission with one larval lineage. This file only contains the model structure — the numerical solver is handled
...
class Params:
    ...
    Transmission (force of infection uses saturating term L/(K+L)):
        ...
        beta_DA : float  Effective contact rate for dogs (day^-1)
...
def model_ode(
    ...
    return np.array([dS_C, dE_C, dI_C, dS_A, dE_A, dI_A, dS_D, dE_DA, dI_DA, dL_A], dtype=float)
