...
def main():
    ...
    required_cols = {"beta_C", "beta_A", "beta_DA", "beta_DB"}
    ...
    # ensure we have at least one full year for the 'prev year' comparisons
    if len(T_EVAL) < 366:
        raise ValueError("Time horizon must be ≥ 1 year to compute last-year prevalence change.")
    ...
    for idx, row in beta_df.iterrows():
        ...
        SD          = sol.y[6, :]
        EDA, IDA    = sol.y[7, :], sol.y[8, :]
        EDB, IDB    = sol.y[9, :], sol.y[10, :]
        L_A, L_B    = sol.y[11, :], sol.y[12, :]
        ...
        # Dogs (both lineages)
        tot_dogs_final = SD[final_idx] + EDA[final_idx] + IDA[final_idx] + EDB[final_idx] + IDB[final_idx]
        tot_dogs_prev  = SD[prev_idx]  + EDA[prev_idx]  + IDA[prev_idx]  + EDB[prev_idx]  + IDB[prev_idx]
        ...
        within_DB = int(abs(prev_I_DB_final - prev_I_DB_prev) <= TOL_PREV)

        # Larval pools
        within_LA = int(L_A[final_idx] != 0 and abs(L_A[final_idx] - L_A[prev_idx]) <= TOL_PREV * abs(L_A[final_idx]))
        within_LB = int(L_B[final_idx] != 0 and abs(L_B[final_idx] - L_B[prev_idx]) <= TOL_PREV * abs(L_B[final_idx]))
        ...
        results.append({
            ...
            # Save full final state (index mapping matches build_y0):
            ...
            "E_DB_final": final_state[9],
            "I_DB_final": final_state[10],
            "L_A_final":  final_state[11],
            "L_B_final":  final_state[12],
