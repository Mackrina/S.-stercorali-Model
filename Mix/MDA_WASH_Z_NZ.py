# solve_MDA_WASH.py
# Run MDA pulses + WASH intervention using two-lineage Model_Z_NZ (13-state system)
# State order (13):
# [S_C, E_C, I_C, S_A, E_A, I_A, S_D, E_DA, I_DA, E_DB, I_DB, L_A, L_B]
...
from Model_Z_NZ import model_ode              # two-lineage signature: model_ode(t, y, params)
...
# File I/O
# --------------------------
EQUIL_CSV = 'filtered_data_Z_NZ.csv'          # must contain betas + *_final columns for all 13 states
OUT_PREV_CSV = 'model_prevalence_MDA_WASH_Z_NZ.csv'
OUT_STATE_CSV = 'model_population_states_MDA_WASH_Z_NZ.csv'
...
# daily RHS evaluations (solver adapts but will return these points)
...
# Reductions are FRACTIONS (e.g., 0.25 means 25% reduction once WASH starts)
DEFAULT_WASH = {
    'wash_beta_red_C': 0.25,  # reduce beta_C
    'wash_beta_red_A': 0.25,  # reduce beta_A
    'wash_f_red_C':   0.00,   # reduce f_C (human shedding/contribution)
...
IDX = {
    ...
    'S_A': 3, 'E_A': 4, 'I_A': 5,
    'S_D': 6,
    'E_DA': 7, 'I_DA': 8,      # dogs infected via lineage A
    'E_DB': 9, 'I_DB': 10,     # dogs infected via lineage B
    'L_A': 11, 'L_B': 12,      # environmental larvae pools
}
...
def apply_mda_pulse(y, params):
    ...
    Instantaneous MDA:
      ...
      - Dogs:   move fraction (c_D * eff_D) of E_DA/I_DA and E_DB/I_DB into S_D
    ...
    # Dogs: both lineages A and B
    move_EDA = frac_D * y[IDX['E_DA']]
    move_IDA = frac_D * y[IDX['I_DA']]
    y[IDX['E_DA']] -= move_EDA
    y[IDX['I_DA']] -= move_IDA
    ...
    y[IDX['S_D']]  += (move_EDA + move_IDA + move_EDB + move_IDB)
...
def rhs_with_wash(t, y, params):
    """
    Model_Z_NZ.model_ode signature: model_ode(t, y, params).
    ...
    if t >= WASH_START_DAY:
        ...
        # reduce acquisition (beta) and/or contribution (f) for HUMANS
    ...
    # Simple non-negativity guard (heuristic)
...
def main():
    ...
    for idx, row in eq.iterrows():
        ...
        # Build y0 from equilibrium row (matches 13-state order)
        y0 = np.array([
            row['S_C_final'], row['E_C_final'], row['I_C_final'],
            row['S_A_final'], row['E_A_final'], row['I_A_final'],
            row['S_D_final'],
            row['E_DA_final'], row['I_DA_final'],
            row['E_DB_final'], row['I_DB_final'],
            row['L_A_final'], row['L_B_final']
        ], dtype=float)

        print(f"Running Particle {idx+1}/{len(eq)}: "
              f"beta_C={params['beta_C']}, beta_A={params['beta_A']}, "
              f"beta_DA={params['beta_DA']}, beta_DB={params['beta_DB']}")
        ...
        # Sequence of segment ends (all pulses + final horizon)
        ...
        for seg_end in segment_ends:
            ...
            if t_eval_seg.size > 0:
                ...
                for j in range(sol.t.size):
                    ...
                    tot_dogs     = (state[IDX['S_D']] + state[IDX['E_DA']] + state[IDX['I_DA']]
                                    + state[IDX['E_DB']] + state[IDX['I_DB']])
                    ...
                    state_rows.append({
                        ...
                        'S_D': state[IDX['S_D']],
                        'E_DA': state[IDX['E_DA']], 'I_DA': state[IDX['I_DA']],
                        'E_DB': state[IDX['E_DB']], 'I_DB': state[IDX['I_DB']],
